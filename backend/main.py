...
from models import TrainingSessionCreate, TrainingSessionUpdate, TrainingSessionResponse, CompleteSessionRequest
...
async def main_page(request: Request):
    """
    Главная страница с интерфейсом тренировки
    """
...
async def complete_session(session_id: int, request: CompleteSessionRequest, db: Session = Depends(get_db)):
    """
    Завершить сессию и проанализировать разговор
    """
    session = db.query(TrainingSession).filter(TrainingSession.id == session_id).first()
    if not session:
        raise HTTPException(status_code=404, detail="Сессия не найдена")
...
async def get_analysis(request: Request, session_id: int, db: Session = Depends(get_db)):
    """
    Показать анализ тренировочной сессии
    """
    session = db.query(TrainingSession).filter(TrainingSession.id == session_id).first()
    if not session:
        raise HTTPException(status_code=404, detail="Сессия не найдена")
...
async def list_sessions(db: Session = Depends(get_db)):
    ...
    """
    sessions = db.query(TrainingSession).order_by(TrainingSession.session_start.desc()).all()
    return sessions
